    ...
    def update(self, events):
        ...
        for event in events:
            if event.type == MOUSEBUTTONDOWN:
                ...
                if all((pos[0] > 255, pos[0] < 559, pos[1] > 452, pos[1] < 513)):
                    return self.value
            if event.type == KEYUP:
                if event.key == K_LSHIFT or event.key == K_RSHIFT or event.key == K_CAPSLOCK: self.shifted = False
            if event.type == KEYDOWN:
                if event.key == K_BACKSPACE: self.value = self.value[:-1]
                elif event.key == K_LSHIFT or event.key == K_RSHIFT or event.key == K_CAPSLOCK: self.shifted = True
